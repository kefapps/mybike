...
export type BiomeVisuals = {
  ...
  materials: Material[];
  coastMaterial: MeshStandardMaterial;
  forestMaterial: MeshStandardMaterial;
...
export function createBiomeVisuals(
  ...
  const coastGeometry = new BoxGeometry(0.65, 2.4, 0.65);
  const trunkGeometry = new CylinderGeometry(0.55, 0.75, 5, 8);
  const canopyGeometry = new ConeGeometry(3.2, 8, 8);
  const grassGeometry = new ConeGeometry(0.32, 1.55, 5);
  const rockGeometry = new DodecahedronGeometry(1, 0);
  const signPlateGeometry = new BoxGeometry(3.6, 1.4, 0.22);
  const signPostGeometry = new CylinderGeometry(0.13, 0.13, 3, 8);
  const farTreeGeometry = new ConeGeometry(4.8, 12, 7);
  const farHillGeometry = new BoxGeometry(36, 6, 18);
  const lighthouseBaseGeometry = new CylinderGeometry(
    1.25,
    1.75,
    10,
    10
  );
  const lighthouseStripeGeometry = new CylinderGeometry(
    1.34,
    1.52,
    0.52,
    10
  );
  const lighthouseLanternGeometry = new CylinderGeometry(
    1.75,
    1.95,
    1.6,
    10
  );
  const lighthouseBeaconGeometry = new BoxGeometry(3.8, 0.85, 3.8);
  const lighthouseRoofGeometry = new ConeGeometry(2.35, 2.1, 10);
  const archPillarGeometry = new BoxGeometry(4.6, 12, 4.4);
  const archBridgeGeometry = new BoxGeometry(15.5, 4.2, 4.6);
  const bermGeometry = new BoxGeometry(18, 2.4, 26);
  const ridgeGeometry = new BoxGeometry(56, 8, 30);
  const canopyBridgeGeometry = new BoxGeometry(26, 4.2, 5.4);
  const nearPostGeometry = new BoxGeometry(0.24, 2.1, 0.24);
  const nearBladeGeometry = new ConeGeometry(0.16, 1.25, 4);
  const nearStoneGeometry = new DodecahedronGeometry(0.45, 0);
  const coastReedGeometry = new BoxGeometry(0.12, 1.85, 0.12);
  const forestFernGeometry = new ConeGeometry(0.42, 1.15, 6);
  const birdWingGeometry = new BoxGeometry(1.8, 0.08, 0.18);
  const birdBodyGeometry = new BoxGeometry(0.5, 0.18, 0.5);
  const humanBodyGeometry = new BoxGeometry(0.62, 1.65, 0.28);
  const humanHeadGeometry = new SphereGeometry(0.3, 10, 8);
  const humanBaseGeometry = new BoxGeometry(0.86, 0.1, 0.42);
  const humanVestGeometry = new BoxGeometry(0.68, 0.34, 0.3);

  const coastMaterial = new MeshStandardMaterial({
    color: 0x4f9fcf,
    roughness: 0.72
  });
  const forestMaterial = new MeshStandardMaterial({
    color: 0x174d2e,
    roughness: 0.82
  });
  ...
  const distantMaterial = new MeshStandardMaterial({
    color: 0x315c46,
    roughness: 0.98
  });
  const lighthouseMaterial = new MeshStandardMaterial({
    color: 0xf4efd9,
    roughness: 0.6
  });
  ...
  const lighthouseRoofMaterial = new MeshStandardMaterial({
    color: 0x35515b,
    roughness: 0.72
  });
  const rockArchMaterial = new MeshStandardMaterial({
    color: 0x8a8f86,
    roughness: 0.9
  });
  ...
  const nearPostMaterial = new MeshStandardMaterial({
    color: 0xf7e8b6,
    roughness: 0.74
  });
  const nearBladeMaterial = new MeshStandardMaterial({
    color: 0x89b66c,
    roughness: 0.96
  });
  const nearStoneMaterial = new MeshStandardMaterial({
    color: 0x696d66,
    roughness: 0.9
  });
  const coastReedMaterial = new MeshStandardMaterial({
    color: 0xb58d55,
    roughness: 0.9
  });
  const forestFernMaterial = new MeshStandardMaterial({
    color: 0x2f7a42,
    roughness: 0.94
  });
  ...
  const humanBodyMaterial = new MeshBasicMaterial({
    color: 0x202825,
    fog: true
  });
  const humanHeadMaterial = new MeshBasicMaterial({
    color: 0xe8d0a8,
    fog: true
  });
  const humanVestMaterial = new MeshBasicMaterial({
    color: 0xf2d36b,
    fog: true
  });
  ...
  const nearRoadMotionDetails = new Group();
  ...
  for (const z of [
    ...
    for (const side of [-1, 1] as const) {
      const post = new Mesh(nearPostGeometry, nearPostMaterial);
  ...
  for (const z of [
    ...
    for (const side of [-1, 1] as const) {
      const blade = new Mesh(nearBladeGeometry, nearBladeMaterial);
  ...
  for (const z of [
    ...
    for (const side of [-1, 1] as const) {
      const stone = new Mesh(nearStoneGeometry, nearStoneMaterial);
  ...
  for (const z of [36, 66, 98, 132, 168]) {
    for (const side of [-1, 1] as const) {
      const reed = new Mesh(coastReedGeometry, coastReedMaterial);
  ...
  for (const z of [248, 286, 326, 368, 414, 462, 516, 574, 640, 712]) {
    for (const side of [-1, 1] as const) {
      const fern = new Mesh(forestFernGeometry, forestFernMaterial);
  ...
  for (const [z, side, offset, scale] of [
    ...
    const base = new Mesh(humanBaseGeometry, humanBodyMaterial);
    base.name = "scenic-life-human-base";
    base.position.y = 0.04;

    const body = new Mesh(humanBodyGeometry, humanBodyMaterial);
    body.name = "scenic-life-human-body";
    body.position.y = 0.82;

    const head = new Mesh(humanHeadGeometry, humanHeadMaterial);
    head.name = "scenic-life-human-head";
    head.position.y = 1.92;

    const vest = new Mesh(humanVestGeometry, humanVestMaterial);
  ...
  for (const z of [52, 86, 122, 158, 196, 236, 278]) {
    for (const side of [-1, 1] as const) {
      const marker = new Mesh(coastGeometry, coastMaterial);
  ...
  const lighthouseLowerStripe = new Mesh(
    lighthouseStripeGeometry,
    coastMaterial
  );
  ...
  const lighthouseUpperStripe = new Mesh(
    lighthouseStripeGeometry,
    coastMaterial
  );
  ...
  const lighthouseLantern = new Mesh(
    lighthouseLanternGeometry,
    lighthouseMaterial
  );
  ...
  const lighthouseBeacon = new Mesh(
    lighthouseBeaconGeometry,
    beaconMaterial
  );
  ...
  for (const x of [-5.2, 5.2]) {
    const pillar = new Mesh(archPillarGeometry, rockArchMaterial);
  ...
  const archBridge = new Mesh(archBridgeGeometry, rockArchMaterial);
  archBridge.name = "scenic-coast-rock-arch-bridge";
  archBridge.position.set(0, 13.2, 0);

  const archCap = new Mesh(rockGeometry, rockArchMaterial);
  ...
  for (const z of [70, 138, 218, 305, 394, 492, 604, 724, 848, 976]) {
    for (const side of [-1, 1] as const) {
      const berm = new Mesh(bermGeometry, bermMaterial);
  ...
  for (const z of [
    ...
    for (const side of [-1, 1] as const) {
      ...
      const tuft = new Mesh(grassGeometry, grassMaterial);
  ...
  for (const z of [235, 288, 345, 403, 466, 532, 601, 672, 746]) {
    for (const offset of [-31, -22, 20, 30]) {
      ...
      const canopy = new Mesh(canopyGeometry, forestMaterial);
  ...
  for (const z of [245, 278, 312, 348, 385]) {
    ...
    const canopyBridge = new Mesh(canopyBridgeGeometry, forestMaterial);
  ...
  for (const [offset, z] of [
    ...
    const x = getRouteCenterXAtZ(route, z) + offset;
    const post = new Mesh(signPostGeometry, trunkMaterial);
    post.name = "scenic-road-sign";
    post.position.set(x, 1.5, z);

    const plate = new Mesh(signPlateGeometry, signMaterial);
  ...
  for (const z of [720, 820, 920, 1030]) {
    for (const offset of [-62, -46, 48, 66]) {
      const x = getRouteCenterXAtZ(route, z) + offset;
      const tree = new Mesh(farTreeGeometry, distantMaterial);
  ...
  for (const [offset, z, scale] of [
    ...
    const x = getRouteCenterXAtZ(route, z) + offset;
    const hill = new Mesh(farHillGeometry, distantMaterial);
